...
- 로그인 [✔] : 임시로 로그인 성공 시 새로고침하며 이동...(로그아웃도)
...
### 210910 (상윤) - login, signup, new-project

📌 `npm i react-hook-form` : form을 위한 라이브러리입니다.

- `App.js`에서 로컬스토리지를 이용하여 로그인, 로그아웃을 구현했습니다. (로그인 시 로컬스토리지에 로그인한 유저정보가 담깁니다.)

- 로그인, 로그아웃 시 (어쩔수없이...) 새로고침 현상이 일어납니다. (wep API로 구현했기때문 -> `document.location.href`)

- 로그인시 Header, 글쓰기, 프로필수정버튼 등 로그인을 했을 때 이용할 수 있거나 보여져야 하는 것들을 업데이트 했습니다.

- `Login.js`, `SignUp.js` : 로그인, 회원가입 api를 구현합니다. 관련된 모달컴포넌트 등이 포함되어있습니다.

❗ 잘 구현 안되는 사항 : `ProjectsToSkill.js`관련) 홈화면에서 스킬버튼을 눌렀을 때 비동기적으로 데이터를 불러오는 현상을 해결하려 노력중입니다.

### 210912 (상윤) - 스킬버튼 비동기 처리

- `useAxios`에서 `useEffect`를 통한 데이터 비동기 처리
